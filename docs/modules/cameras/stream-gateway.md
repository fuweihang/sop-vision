# Stream Gateway Adapter 与状态投影

> 相关文档：[Cameras 基础能力](foundation.md)、[MediaMTX 契约](mediamtx-contract.md)
...
具体实现。当前 Camera 创建用例已消费该 dependency；共享 Desired State 构造和周期恢复由
[媒体对账](media-reconciliation.md)负责。其余 Camera 请求会在各自实现后复用同一 Port。
...
`fetch_config_path_snapshot()` 为[媒体对账](media-reconciliation.md)提供远端配置观察：

- 受管 Path 的所有权遵循 [Cameras 模块的 UUID Path 规则](README.md#模块边界)。其 `source` 或
...
采集器应按 [日志事件表](../backend-logging/events.md#已注册事件)读取 `stream_gateway.io`；即使
