...
- 默认源单选不做乐观更新。列表 Card 只跟随列表响应中的默认 Source；Detail 不读取默认源，始终按
  保存顺序选择第一路可播放 Source，临时选择仍可播放时保持不变。
...
- PUT 和 PATCH Mutation 都禁止自动重试。结果未知时不做乐观更新，并立即重新获取列表和详情。
  PUT 保留当前表单，重新读取不覆盖草稿或自动判定成功；再次保存必须由用户确认后才发送新请求。
  PATCH 继续显示提交前选择，详情重新读取成功后解除未知提示；重新读取失败时保持提示，但用户再次
  单选可直接发送一条新的明确目标状态请求。
