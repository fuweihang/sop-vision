...
本任务只补齐下文明确列出的验收夹具和长期文档，不补做缺失业务能力。自动化或浏览器验收发现
01–04 的业务实现失败时，立即暂停 05，回到对应事实所有者修复并重新执行验收；不能在这里增加
第二份规则、临时兼容分支或只为测试通过的绕过。
...
- 在现有 Cameras MSW 场景闭包内补齐最小 PUT 状态：保存连接字段以及现有两路 Source 的名称、
  后缀、顺序和默认标记，按 `source_id` 保留既有运行态字段，并让后续列表/详情 GET 投影更新结果。
  该状态只服务本次浏览器验收，不复制 Backend 字段校验、不持久化到浏览器，也不扩展成通用可变
  Mock Store；Source 新增/删除继续由自动化测试覆盖。
- 浏览器中修改连接字段以及现有 Source 的名称、后缀和顺序，保存后必须从重新读取的 GET 看到变化；
  再完成默认源 PATCH、Detail 保持排序第一路或临时 Source、离线默认源、Card 与 Detail 同源共享及
  最后一个 Lease 释放。
...
- 按实际变化更新 `camera-detail.md`、`media-reconciliation.md`、`whep-player.md` 等事实所有者，并检查
  其他当前能力文档是否仍把 Detail 写成默认源优先；只有形成独立业务用例时才新增相应能力文档。
- 更新仍待执行的 `11-release-gates/README.md`，确保发布验收使用“Detail 按排序选择第一路可播放
  Source，默认源只控制 Card”的当前规则；不改变 11 负责的真实依赖、故障、浏览器和容量边界。
...
2. 在现有 MSW 场景闭包内补齐最小 PUT 状态和回归测试，证明成功 PUT 后重新读取能得到更新后的连接
   字段、现有 Source 配置和顺序；不在 Mock 中复制 Backend 校验。
3. 配置独立 PostgreSQL，执行 Backend、Frontend、契约、敏感数据和静态检查。若失败属于 01–04
   业务实现，暂停 05 并回到对应事实所有者修复；修复通过后从完整验收重新开始。
4. 执行可控对账恢复测试和双路 synthetic WHEP 浏览器冒烟；业务行为失败时按上一步暂停和回退。
5. 在全部行为通过后，根据实际代码更新 Cameras 当前能力、相关事实所有者文档和任务 11 的选源规则。
6. 新增 `docs/changes/` 交付记录，检查 Markdown 相对链接、重复规则和敏感信息。
7. 最后移除上级计划中的 09 条目和本计划目录，确认 10 成为下一项待实施能力。
...
另外执行双路 synthetic WHEP 浏览器冒烟，确认 PUT 后重新读取显示更新结果，并检查新增/修改
Markdown 的全部相对链接。必须确认 PostgreSQL 集成测试实际执行而不是 skip。

## 完成标准

- PUT/PATCH、跨端契约、数据库事务/并发、媒体 diff/恢复、编辑 Dialog、默认源和 Session 联动组合
  后全部通过。
- 现有 MSW 场景只维护验收所需的最小 PUT 状态，重新读取能确认连接字段、现有 Source 配置和顺序，
  且没有新增通用 Mock Store 或第二份 Backend 校验。
- 请求、响应、Problem、日志、通知、错误上报和 Mutation cache 保持新旧密码、Source 后缀和完整
  RTSP URL 的既有敏感数据边界。
- Cameras 模块文档和交付记录准确反映实际实现，不复制公共规则或留下“编辑未实现”旧说明。
- 09 已从剩余计划和文件系统移除，10 成为下一项待实施任务；11 已使用当前 Detail 排序选源规则，
  真实依赖边界保持不变。
