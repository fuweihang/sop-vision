...
- UI 实施前读取 `docs/design-system/agent-guidelines.md`、`catalog.json`、
  `docs/design-system/specs/page-patterns.md`、布局和交互规格，并检查现有
  `frontend/src/components/ui/`。不得手动编辑 `frontend/src/routeTree.gen.ts`。
...
- Entity Header 展示 Camera 名称，沿用 Shell 已有返回 Cameras 链接；启动预览和编辑使用禁用的
  占位按钮，不在操作按钮旁重复展示状态。
...
- 首次加载骨架必须匹配本任务页面，复用现有共享详情骨架；不得为单一页面复制整套共享骨架。
...
- 完成代码与全套验证后：
  ...
  5. 从 `docs/plans/cameras-mvp/README.md` 移除 06，但保留整个 `06-camera-detail/` 目录，等待用户在
     后续流程中自行清理。
...
3. 实现只读 Header、连接信息、预览区域和 Source 列表，调整详情骨架的最小参数。
4. 补页面行为、无操作控件、敏感数据、响应式和可访问性测试。
5. 运行 Frontend 与跨端门禁；全部通过后更新当前能力、变更记录和计划链接，并从 Cameras MVP
   总计划移除 06；保留 `06-camera-detail/` 目录，等待用户后续自行清理。
