...
from tests.support.cameras.builders import CameraBuilder, CameraListBuilder, FixedClock
...
async def test_列表查询先计数再分页结束事务后只投影一次快照() -> None:
...
async def test_列表查询空页返回真实总数且不访问网关() -> None:
...
async def test_列表查询对总数和分页应用相同搜索条件() -> None:
    """搜索条件必须同时用于总数和分页读取，当前页信息不能退回默认值。"""

    cameras = CameraListBuilder().build(3)
    uow = await committed_reader(cameras)
    gateway = FakeStreamGateway(RuntimePathSnapshot(paths=(), checked_at=SNAPSHOT_AT))

    result = await list_cameras(
        CameraListCriteria(q="分页 Camera"),
        2,
        2,
        uow=uow,
        stream_gateway=gateway,
    ...
    assert tuple(item.camera.camera_id for item in result.items) == (cameras[2].camera_id,)
    assert uow.operation_log[:2] == [
        "repository.count:分页 Camera",
        "repository.list:分页 Camera:2:2",
    ]
    assert gateway.runtime_snapshot_count == 1
...
async def test_列表查询对已知网关失败降级且整页共用时间戳(
...
async def test_列表查询将数据损坏转换为不含身份和异常上下文的错误(
...
async def test_列表查询继续抛出数据库失败且不访问媒体网关(failure_stage: str) -> None:
...
async def test_列表查询继续抛出任务取消(cancel_stage: str) -> None:
...
async def test_列表查询结束事务后继续抛出未知网关错误() -> None:
    """未知媒体程序错误不能伪装成离线列表。"""
