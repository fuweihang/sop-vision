...
async def test_detail_uses_one_snapshot_and_shared_camera_status_rules(
...
async def test_detail_degrades_supported_gateway_failures_after_ending_transaction(
...
async def test_detail_not_found_ends_transaction_without_accessing_gateway() -> None:
...
async def test_detail_converts_corrupted_aggregate_to_safe_error_and_one_log(caplog) -> None:
...
async def test_detail_propagates_task_cancellation(cancel_stage: str) -> None:
...
async def test_detail_propagates_unknown_gateway_error_after_ending_transaction() -> None:
