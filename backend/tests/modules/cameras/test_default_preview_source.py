...
async def test_set_default_preview_source_saves_complete_aggregate_without_runtime_dependency() -> (
    None
):
...
async def test_reselecting_current_default_still_advances_camera_updated_at() -> None:
...
async def test_foreign_source_rolls_back_without_saving() -> None:
...
async def test_missing_and_corrupted_camera_end_transaction_with_safe_errors() -> None:
...
async def test_database_failure_rolls_back_uncommitted_default_change(failure_stage: str) -> None:
    """读取、保存或提交失败都恢复旧聚合，并向上保留数据库不可用错误。"""

    camera = CameraBuilder().build(source_count=2)
    store = await store_camera(camera)

    class FailingCommitUnitOfWork(FakeCameraUnitOfWork):
        """提交前失败，不发布 Fake 工作副本。"""
    ...
    uow = (
        FailingCommitUnitOfWork(store) if failure_stage == "commit" else FakeCameraUnitOfWork(store)
    )
...
async def test_commit_cancellation_propagates_and_rolls_back_working_copy() -> None:
...
async def test_server_clock_invariant_is_not_mislabelled_as_persisted_corruption() -> None:
