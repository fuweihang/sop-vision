...
from tests.support.cameras.builders import CameraBuilder, uuid4_from_index

CHECKED_AT = datetime(2026, 8, 27, 8, 0, tzinfo=UTC)


def _projection(source_id: UUID, *, online: bool) -> SourceRuntimeProjection:
...
def test_汇总Camera运行状态时使用全部视频源投影(
    ...
    summary = summarize_camera_runtime(
        camera,
        tuple(
            _projection(source.source_id, online=online)
...
@pytest.mark.parametrize(
    "mutation",
    ["missing", "reordered", "foreign"],
    ids=["投影缺失", "投影顺序错误", "外来视频源"],
)
def test_汇总Camera运行状态时拒绝不匹配的投影(mutation: str) -> None:
    """数量、顺序或 ID 错配都不能按数组位置静默拼到错误 Source。"""

    camera = CameraBuilder().build(source_count=2)
    projections = [_projection(source.source_id, online=False) for source in camera.sources]
    if mutation == "missing":
        projections.pop()
    elif mutation == "reordered":
        projections.reverse()
    else:
        projections[1] = _projection(uuid4_from_index(999), online=False)
