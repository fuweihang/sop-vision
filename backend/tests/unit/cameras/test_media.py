...
def test_构造媒体期望状态时保留顺序并编码凭据() -> None:
...
def test_构造媒体期望状态时拒绝其他Camera的视频源() -> None:
...
def test_媒体差异忽略名称顺序和默认源变化() -> None:
...
def test_媒体差异先按新顺序确保变更项再按旧顺序释放删除项() -> None:
...
@pytest.mark.parametrize(
    "connection_field",
    ["ip_address", "rtsp_port", "username", "password"],
    ids=["IP地址", "RTSP端口", "用户名", "密码"],
)
def test_任一连接字段变化时媒体差异确保所有现有视频源(connection_field: str) -> None:
    ...
    updated = camera.update_configuration(
        ...
        password=(
            "new-password"
            if connection_field == "password"
            else camera.credentials.password.reveal()
        ),
        sources=tuple(
            CameraSourceChange(
