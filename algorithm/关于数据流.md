...
## 7. algorithm Demo 的 ROI 配置通道

当前 `algorithm` 目录中的第一版 algorithm demo 为了保持轻量，ROI 更新暂时只使用 Redis Pub/Sub：

```text
ROI publisher
    │ PUBLISH vision:config:roi:algorithm-demo
    ▼
Redis
    │ SUBSCRIBE
    ▼
...
- algorithm 对整帧执行 YOLO，仅保留 bbox 中心点在多边形内部或边界上的目标。
- `enabled=false` 且 `points=[]` 表示清除 ROI、恢复全画面检测。
- 非法消息不会覆盖当前合法 ROI。
- 此 demo 不保存 ROI 最新值。algorithm 重启、Redis 断线或订阅尚未建立时发布的消息都会丢失，启动后需重新发布。
