...
任务级算法参数和 ROI 不经过 Redis，也不由 Worker 控制接口传入。外部客户端先根据
Daemon 公开的 JSON Schema 生成配置，再把任务参数保存到 PostgreSQL。模型路径和 Redis
地址不属于任务参数：它们分别按 Worker 类型和全局统一配置在外围 `config.toml` 中：

```text
Qt / 平台客户端 ──UPSERT──> worker_task_parameters
        │                         │
        └────start/reload────────> Algorithm Daemon
config.toml ─────────────────────>│ 注入模型路径和统一 Redis 地址
                                  │ 读取、完整校验并启动
                                  ▼
                               AIWorker
```
...
- 修改 TOML 后也通过任务 `reload` 生效；配置文件无效时正在运行的 Worker 不会被停止。
- 任务参数包含 `model_path` 或 `redis_url` 时校验失败，不能覆盖外围 TOML。
